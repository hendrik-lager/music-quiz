...
# ---------------------------------------------------------------------------
# Standalone (non-HA) helpers
# ---------------------------------------------------------------------------

def discover_playlists(playlist_dir: "Path") -> list[dict]:
    """Synchronous playlist discovery for standalone mode."""
    from pathlib import Path as _Path  # noqa: PLC0415

    playlist_dir = _Path(playlist_dir)
    playlists: list[dict] = []
    ...
    for json_file in playlist_dir.glob("**/*.json"):
        try:
            rel = json_file.relative_to(playlist_dir)
            source = "community" if rel.parts and rel.parts[0] in ("community", "user") else "bundled"
            ...
            playlists.append({
                ...
                "tags": data.get("tags", []),
                "song_count": len(songs),
                "spotify_count": _count("uri_spotify", URI_PATTERN_SPOTIFY) + _count("uri", URI_PATTERN_SPOTIFY),
                "apple_music_count": _count("uri_apple_music", URI_PATTERN_APPLE_MUSIC),
                "youtube_music_count": _count("uri_youtube_music", URI_PATTERN_YOUTUBE_MUSIC),
                "tidal_count": _count("uri_tidal", URI_PATTERN_TIDAL),
                "deezer_count": _count("uri_deezer", URI_PATTERN_DEEZER),
            ...
            playlists.append({
                ...
                "deezer_count": 0,
                "is_valid": False,
                "errors": [f"Invalid JSON: {err}"],
            })

    _LOGGER.debug("Found %d playlists in %s", len(playlists), playlist_dir)
    return playlists
