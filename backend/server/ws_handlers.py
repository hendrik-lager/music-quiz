...
from server.serializers import build_state_message
...
def _is_ha_authenticated(handler: BeatifyWebSocketHandler, data: dict) -> bool:
    """Return True if the message carries a valid admin token or Spotify session.

    In standalone mode, authentication uses the game's admin_token or a
    valid Spotify access token stored in the app data.
    """
    # Check game admin_token
    token = data.get("ha_token") or data.get("admin_token")
    if not token:
        return False

    # Allow if it matches the Spotify access token (admin is logged in via Spotify)
    app_data = handler._app_data
    spotify_token = app_data.get("spotify_access_token")
    if spotify_token and token == spotify_token:
        return True

    # Allow if it matches the game admin token (admin set up the game)
    game = app_data.get("game")
    if game and game.admin_token and token == game.admin_token:
        return True

    _LOGGER.warning("[WS auth] admin_connect rejected: invalid token")
    return False
...
async def admin_next_round(
    ...
    if game_state.phase == GamePhase.PLAYING:
        ...
        if game_state.last_round:
            stats_service = handler._app_data.get("stats")
            if stats_service:
...
async def handle_report_data(
    ...
    reports_path = (
        Path(handler._app_data.get("data_dir", ".")) / "data_quality_reports.json"
    )
    try:
